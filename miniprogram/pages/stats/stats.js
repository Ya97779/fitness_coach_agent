...
Page({
  ...
  drawLineChart(logs) {
    ...
    query.exec(res => {
      ...
      // 绘制摄入线
      this.drawLine(ctx, intakes, maxVal, logs.length, padding, chartW, chartH, '#4CAF50')
      // 绘制消耗线
      this.drawLine(ctx, burns, maxVal, logs.length, padding, chartW, chartH, '#2196F3')
  ...
  drawBarChart(logs) {
    ...
    query.exec(res => {
      ...
      logs.forEach((l, i) => {
        ...
        ctx.setFillStyle(net >= 0 ? '#4CAF50' : '#2196F3')
