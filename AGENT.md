...
只有 `confirmed=true` 且 `status=active` 的语义事实进入 Prompt；未确认事实保留为
`candidate`，不能作为用户画像使用。客户端只能提交记忆键、值和有效期，来源与
确认状态由服务端决定。`RequestLedger` 按用户和请求编号记录执行状态与完整结果，
跨 Gunicorn worker 阻止重复执行；失败的请求编号不会自动重试，以免重复写入饮食/
运动记录。清空用户数据会同时删除工作记忆、情景记忆、语义记忆和请求账本，保留
基础个人资料；有进行中的请求时返回 409。
...
共 197 项通过。RAG 测试可能打印外部模型/提示词降级日志，但不影响该基线的
...
- 不直接修改 `showdown.js` 等第三方或供应商代码，除非任务就是升级依赖，并能说明来源和验证范围。
