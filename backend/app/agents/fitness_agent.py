        ...
        try:
            if stream:
                ...
                for _retry in range(3):
                    ...
                    print(f"[fitness_agent] 流式调用第{_retry+1}轮, messages={len(chat_history)}", flush=True)
                    raw_idx = 0
                    ...
                    print(f"[fitness_agent] 流式完成: total_chunks={raw_idx}, content_chunks={chunk_count}, 总长度={len(total_content)}", flush=True)
                    ...
                    if accumulated and getattr(accumulated, 'tool_calls', None):
                        print(f"[fitness_agent] 流式返回 tool_calls，执行工具后重试...", flush=True)
                        for tc in accumulated.tool_calls:
                            tc_name = tc['name']
                            tc_args = tc['args']
                            print(f"[fitness_agent] 工具调用: {tc_name}({tc_args})", flush=True)
                            for t in fitness_tools:
                                if t.name == tc_name:
                                    try:
                                        tc_result = t.invoke(tc_args)
                                        print(f"[fitness_agent] 工具结果: {tc_name} → {tc_result}", flush=True)
